/** Create the lamlinks_invoice_queue table. */
import "./env";
...
async function main() {
  ...
  const sql = readFileSync(join(__dirname, "sql", "lamlinks-invoice-queue.sql"), "utf-8");
  const resp = await fetch(mgmtUrl, {
    method: "POST",
    headers: { Authorization: `Bearer ${mgmtToken}`, "Content-Type": "application/json" },
    body: JSON.stringify({ query: sql }),
  });
  console.log(`HTTP ${resp.status}`); console.log((await resp.text()).slice(0, 400));
