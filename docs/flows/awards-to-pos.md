...
| `nsn_review_overrides` | read on PO gen, write on review save AND pre-award sourcing modal | `(nsn, vendor)` UNIQUE → `unit_of_measure`, `unit_cost`, `supplier_sku`, `notes` (legacy), `reviewed_by`, `reviewed_at`. Review-once state: `generate-pos` checks this first, falls back to waterfall only when no override. Tagged `cost_source = "reviewed override by …"` when used. Writable from both `/orders` Review tab (post-award) and `/solicitations` Sourcing button (pre-award). |
| `nsn_sourcing_notes` | append-only from sourcing modal | `(nsn, vendor [nullable], note, author, created_at)`. Every "Add a note" in the sourcing modal creates a new row — never overwrites. Modal reads the latest 50 for the NSN (newest first) so Abe sees history like "blocked last quarter / still blocked / unblocked 2026-08-01" as distinct entries. |
