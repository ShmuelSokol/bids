-- dbo.test_invoices_new


CREATE proc [dbo].[test_invoices_new]
...
insert into temp_debug_log (dt) values(CURRENT_TIMESTAMP)
...
'select
	...
	, ltrim(rtrim(isnull(U_VOUCHER_REQUIRED, ''N''))) as U_VOUCHER_REQUIRED
	, ltrim(rtrim(isnull(U_MAILING_ADDRESS_IN_NOTES, ''N''))) as U_MAILING_ADDRESS_IN_NOTES
	, ltrim(rtrim(isnull(U_SEND_0_BALANCE_INVOICES, ''N''))) as U_SEND_0_BALANCE_INVOICES
	, ltrim(rtrim(isnull(U_OTHER_INVOICING_INSTRUCT, ''''))) as U_OTHER_INVOICING_INSTRUCT
	, ltrim(rtrim(isnull(U_EXCLUDE0BALANFROMDAILYREV, ''N''))) as U_EXCLUDE0BALANFROMDAILYREV
	, ltrim(rtrim(isnull(U_ALWAYSEXCLUDEFROMDAILYREV, ''N''))) as U_ALWAYSEXCLUDEFROMDAILYREV
from
	openquery (
		SZY_WinSol_ODBC, 
		''select
			...
			, c.U_VOUCHER_REQUIRED, c.U_MAILING_ADDRESS_IN_NOTES, c.U_SEND_0_BALANCE_INVOICES, c.U_OTHER_INVOICING_INSTRUCT, c.U_EXCLUDE0BALANFROMDAILYREV
			, c.U_ALWAYSEXCLUDEFROMDAILYREV
		from
...
begin
	set @sql =
	'select
		''' + @cc + ''' as CUSTMR, MONEY_ON_ACCOUNT
	from
		openquery (
			SZY_WinSol_ODBC, 
			''select
				sum(a.INVCAMT + a.TOTPAY) as MONEY_ON_ACCOUNT
			from
				arar a
			where
				a.CUSTMR = ''''' + @cc + '''''
